...
export default {
  ...
  methods: {
    ...
    createFilters(objects, propertyNames, excludeProperties) {
      const filters = [];
      console.log(objects)
      objects.forEach((object) => {
        ...
        Object.keys(object).forEach((key) => {
          ...
          if (filter) {
            if (!filter.values.includes(object[key])) {
              filter.values.push(object[key]);
            }
          } else {
            filters.push({ name: displayName, values: [object[key]], objectKey: key });
          }
